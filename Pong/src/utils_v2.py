...
class Image:
    ...
    def render_image(self, path, convert, transform):
    ...
    def _render_image(self, path, convert, transform, colorkey=None, mode=None):
        # try to load and transform image
        self.path = path
        try:
            if convert:
...
class Tile:
    ...
    '''def generate_tiles(qunatity, x_start, y_start, width, height, kind):
        # make tiles
        world_tiles = []
        x, y = x_start, y_start
        for i in range(qunatity):
            if x == WIDTH:
                x = 0
                y -= width
            tile = Tile(width, height, tiles[kind]).make_tile(x, y, True, True)
...
class HUD_Button:
    ...
    def __init__(self, image, pos, color=None):
...
class TickBox:
    ...
    def __init__(self, pos, size=None, bg_color=None, tick_color=None, border=False, border_clr=None):
        ...
        self.bg_button = HUD_Button(pg.Surface(self.size), self.pos, self.bg_clr)

        # visual tick button
        self.tick_size = (self.size[0]-10, self.size[1]-10)
        self.tick_pos = (self.bg_button.rect.x+5, self.bg_button.rect.y+5)
        self.tick_rect = HUD_Button(pg.Surface(self.tick_size), self.tick_pos, self.tick_clr)
