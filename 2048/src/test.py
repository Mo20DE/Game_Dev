...
class AlgoMenu:
    ...
    def getBestGreedyMove(self, board, start_move, depth) -> int:
        ...
        if self.tryInsertTile(bd):
            ...
            for _ in range(depth-1):
                ...
                if not self.tryInsertTile(bd): break
    ...
    def on_greedy_btn_click(self, board, samples, depth):
        ...
        for _ in range(samples):
            rew_samples[0] += self.getBestGreedyMove(first_boards[0], "up", depth)
            rew_samples[1] += self.getBestGreedyMove(first_boards[1], "down", depth)
            rew_samples[2] += self.getBestGreedyMove(first_boards[2], "left", depth)
            rew_samples[3] += self.getBestGreedyMove(first_boards[3], "down", depth)
